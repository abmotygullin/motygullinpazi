...
/* ----------------------------------------------------------------------------------------------- */
 ...
 /* ----------------------------------------------------------------------------------------------- */
/*               реализация генератора, использующего шифр Магма в режиме счетчика                  */
/* ----------------------------------------------------------------------------------------------- */
/** @brief Класс для хранения данных о шифре и текущем значении счетчика */
 typedef struct magma_random_data {
     ...
     ak_bckey magma;
     /** @brief Текущее внутренее состояние генератора */
     ak_uint64 current;
 ...
 /* ----------------------------------------------------------------------------------------------- */
  /**
   * @brief  Инициализирует генератор на основе Магмы.
   *
   * @param rnd Генератор на основе Магмы.
   * @param ptr Указатель на данные для инициализации генератора.
   * @param size Количество параметров для инициализации.
   * @return int В случае успеха, функция возвращает \ref ak_error_ok. В противном случае
             возвращается код ошибки.
   */
...
/* ----------------------------------------------------------------------------------------------- */
/**
 * @brief Генерирует 8 следующих байт с помощью генератора на основе Магмы.
 * @param rnd Генератор на основе Магмы.
 * @return int В случае успеха, функция возвращает \ref ak_error_ok. В противном случае
             возвращается код ошибки.
 */
 static int ak_random_magma_next(ak_random rnd)
 {
     ...
     ak_bckey_encrypt_ecb(ctx->magma, &ctx->current, ctx->last_bytes, 8);
     ctx->current++;
     ctx->current_index = 0;
 ...
 /* ----------------------------------------------------------------------------------------------- */
 /**
  * @brief Функция выработки последователности псевдо-случайных байт генератором на основе Магмы.
  *
  * @param rnd Генератор на основе Магмы.
  * @param buffer Указатель на вырабатываемую последовательность.
  * @param size Размер после5довательности в байтах.
 ...
 /* ----------------------------------------------------------------------------------------------- */
 /**
  * @brief Функция освобождения внутреннего состояния генератора на основе Магмы.
  *
  * @param rnd Контекст создаваемого генератора.
  * @return int В случае успеха, функция возвращает \ref ak_error_ok. В противном случае
             возвращается код ошибки.
 ...
 /* ----------------------------------------------------------------------------------------------- */
 /**
  * @param generator Генератор на основе магмы
    @param magma Используемый контекст Магмы
    @return int В случае успеха, функция возвращает \ref ak_error_ok. В противном случае
             возвращается код ошибки.                                                              */
